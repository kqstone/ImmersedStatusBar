...
public class OnContentChangedHook extends XC_MethodHook {
	...
	private void update(Activity activity) {
		...
		if (type != WindowType.Normal)
			return;
		boolean hasProfile = (Boolean) XposedHelpers
				.getAdditionalInstanceField(activity, "mHasProfile");
		if (hasProfile)
			return;
		int changeTimes = (Integer) XposedHelpers.getAdditionalInstanceField(
				activity, "mContentChangeTimes");
		XposedHelpers.setAdditionalInstanceField(activity,
				"mContentChangeTimes", changeTimes + 1);
		if (changeTimes <= 1) {
			return;
		}

		Utils.log("Content changed for " + changeTimes
				+ " times, re-tint statusbar");
